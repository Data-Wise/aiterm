...
**Status:** Planning
**Target:** After v0.4.0 stabilization
...
| Command | Description |
|---------|-------------|
| `ait release check` | Pre-release validation (version, tests, changelog) |
| `ait release status` | Show current release state and pending changes |
| `ait release pypi` | Build and publish to PyPI |
| `ait release homebrew` | Update Homebrew formula in tap |
| `ait release tag` | Create annotated git tag |
| `ait release notes` | Generate release notes from commits |
| `ait release full` | Full release workflow (check → tag → pypi → homebrew) |
...
### v0.5.0 MVP
- [ ] `ait release check` validates release readiness
- [ ] `ait release status` shows current state
- [ ] `ait release pypi` publishes to PyPI
- [ ] `ait release full` runs complete workflow
- [ ] Tests for all release commands
- [ ] Documentation updated

### Stretch Goals
- [ ] `ait release homebrew` updates tap
